...
const RegisterErrorModal = ({ msg }: props) => {
  const { setopenErrorRegisterModal } = useAuth();
  return (
    <motion.div
      initial={{
        opacity: 0,
      ...
      className='w-full overflow-hidden h-[100vh] z-10 absolute inset-0 backdrop-blur-sm grid place-items-center '
    >
      <article className='bg-white border rounded-lg max-w-[500px] w-[95%] pb-4 shadow relative '>
        <button
          ...
          onClick={() => setopenErrorRegisterModal(false)}
        >
        ...
        <div className=' flex flex-col  text-sm space-y-3  mx-4 '>
          <button
            className=' w-full py-1 border  bg-[#3b49de] text-white rounded shadow '
            onClick={() => setopenErrorRegisterModal(false)}
          >
            cancel
          </button>
